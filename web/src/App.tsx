...
function RunCard({ run }: RunCardProps) {
  return (
    <article className={`card card--run card--${run.status}`}>
      ...
      <div className="shell">
        <header className="hero">
          ...
          <div className="hero__status">
            <div className={`status-card status-card--${health.status}`}>
              ...
              <h2>{health.status === 'ok' ? 'Connected' : health.status === 'error' ? 'Disconnected' : 'Checking'}</h2>
        ...
        <section className="runs">
          <div className="runs__header">
            <h2>Recent runs</h2>
            <p>Newest first. Click refresh to pull the latest data.</p>
          </div>
          ...
          <div className="runs__grid">
            {sortedRuns.length === 0 ? (
              <div className="empty">
                <h3>No runs yet</h3>
                <p>Start with <code>autorunner run</code> to populate the timeline.</p>
              </div>
            ) : (
